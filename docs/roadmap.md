...
- **MP1**: Memory (ontology + persistence)
- **MP2**: Regulator (policy + constraints + mutations)
- **MP3**: Cortex CLI + Sensorium stub (orchestration + status/signal/act)

## Next micro-projects (6–10)

### MP4 — Episode Lifecycle (More Real, Still Minimal)

**Goal**: Make Episodes meaningfully “temporary” with explicit closure and timestamps, without expanding into a planner.

**Scope**

- Add minimal fields to `Episode` (e.g., `openedAt`, optional `closedAt`, optional `closureNoteId`).
- Keep status machine small and explicit (if adding statuses, use const assertions and narrow transitions).

**Acceptance**

- New fields are persisted and migrated cleanly.
- Tests cover: creating an episode sets `openedAt`; closing sets `closedAt`; invalid transitions are rejected.
...
- Add a Regulator mutation that closes an episode with a `closureNote` (or note id + content).
- Keep it cybernetically honest: “learning exists only if an explicit artifact exists.”

**Acceptance**

- Closing an episode can attach a `closureNoteId`.
